...
export class TermsOfUseComponent implements OnInit {
    ...
    uiLabels: any = this.languageService.getUiLabels(AppConstant.COMP.TERMS_OF_USED, AppConstant.UI_LABEL_TEXT);
    componentCode!: string;
    isSystemAdmin: boolean = false;
    ngOnInit(): void {
        this.isSystemAdmin = this.utilService.languageEditMode();
        this.componentCode = AppConstant.COMP.TERMS_OF_USED;
        this.isAdmin = (this.isAdmin) ? this.isAdmin : false;
        if (!this.isAdmin) {

            this.systemOverviewService.getStaticPage(AppConstant.Tabs[5].staticPageId, ((data: StaticPageFetch) => {
                if (data) {
