...
@Component({
    selector: 'app-company-using-it',
    templateUrl: './company-using-it.component.html',
    styleUrls: ['./company-using-it.component.css']
})
export class CompanyUsingItComponent implements OnInit {
    ...
    uiLabels: any = this.languageService.getUiLabels(AppConstant.COMP.COMPANY_USING_IT_TAB, AppConstant.UI_LABEL_TEXT);
    componentCode!: string;
    isSystemAdmin: boolean = false;
    ngOnInit(): void {
        this.isSystemAdmin = this.utilService.languageEditMode();
        this.componentCode = AppConstant.COMP.COMPANY_USING_IT_TAB;
        this.isAdmin = (this.isAdmin) ? this.isAdmin : false;

        if (!this.isAdmin) {

            this.systemOverviewService.getStaticPage(AppConstant.Tabs[3].staticPageId, ((data: StaticPageFetch) => {
                if (data) {

                    this.staticPage = data;
                    this.loadStaticContent(data);
                }
