...
export class PrivacyPolicyComponent implements OnInit {
    ...
    uiLabels: any = this.languageService.getUiLabels(AppConstant.COMP.PRIVACY_POLICY, AppConstant.UI_LABEL_TEXT);
    componentCode!: string;
    isSystemAdmin: boolean = false;
    ngOnInit(): void {
        this.isSystemAdmin = this.utilService.languageEditMode();
        this.componentCode = AppConstant.COMP.PRIVACY_POLICY;
        this.isAdmin = (this.isAdmin) ? this.isAdmin : false;

        if (!this.isAdmin) {

            this.systemOverviewService.getStaticPage(AppConstant.Tabs[6].staticPageId, ((data: StaticPageFetch) => {
                if (data) {
